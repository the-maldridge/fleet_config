...
resource "routeros_interface_bridge_vlan" "lan" {
  ...
  bridge   = routeros_interface_bridge.br0.name
  vlan_ids = tostring(routeros_interface_vlan.vlan["lan0"].vlan_id)
  untagged = [each.key]
}
...
resource "routeros_interface_bridge_vlan" "wan" {
  ...
  bridge   = routeros_interface_bridge.br0.name
  vlan_ids = tostring(routeros_interface_vlan.vlan["wan0"].vlan_id)
  untagged = [each.key]
}
...
resource "routeros_interface_bridge_vlan" "peer" {
  ...
  bridge   = routeros_interface_bridge.br0.name
  vlan_ids = tostring(routeros_interface_vlan.vlan["peer0"].vlan_id)
  untagged = [each.key]
}
